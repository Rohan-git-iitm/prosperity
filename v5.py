...
class Trader:
    ...
    # ── ACO hyperparameters ────────────────────────────────────
    # spread=3, skew=2: reverted from grid-search values (spread=2, skew=1).
    # Live test confirmed: fill count is identical at spread=2 vs 3.
    # Tighter spread just reduces revenue per fill (avg 4.30 vs 5.46 ticks).
    # Grid search overestimated fill frequency because its fill model counts
    # snapshot crossings, not bots actively choosing to trade at our level.
    #
    # agg_size=15, buy_limit=70: kept from grid search.
    # These affect free-money aggressive takes — deterministic in backtest too.
    ACO_FAIR             = 10_000
    ACO_SPREAD_TICKS     = 3       # reverted: wider = more revenue per fill
    ACO_SKEW_STRENGTH    = 2       # reverted: must stay < SPREAD_TICKS
    ACO_IMBALANCE_WT     = 0.0     # confirmed noise — disabled
    ACO_PASSIVE_SIZE     = 15      # p75 of bot level-1 volume
    ACO_AGGRESSIVE_SIZE  = 15      # grid-confirmed: bigger takes = more free money
    ACO_BUY_LIMIT        =  70     # grid-confirmed: wider threshold helps
    ACO_SELL_LIMIT       = -70
